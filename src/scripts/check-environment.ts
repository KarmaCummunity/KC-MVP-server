...
if (!environment) {
  result.valid = false;
  result.errors.push('❌ ENVIRONMENT לא מוגדר! צריך להיות development או production');
} else {
  result.info.push(`✅ ENVIRONMENT: ${environment}`);
  
  if (environment !== 'development' && environment !== 'production') {
    result.valid = false;
    result.errors.push(`❌ ENVIRONMENT="${environment}" לא תקין! צריך להיות development או production`);
  }
}
...
if (!nodeEnv) {
  ...
  if (environment && nodeEnv !== environment) {
    result.warnings.push(`⚠️  NODE_ENV (${nodeEnv}) שונה מ-ENVIRONMENT (${environment})`);
  }
}
...
if (!databaseUrl) {
  ...
  if (environment === 'development') {
    ...
    if (databaseUrl.includes('mmWLXgvXF') || databaseUrl.includes('postgres-a3d6beef')) {
      result.info.push('✅ DATABASE_URL נראה כמו של Development (סיסמה או host נכונים)');
    } else if (databaseUrl.includes('RHkhivARk')) {
      result.valid = false;
      result.errors.push('🚨 DATABASE_URL נראה כמו של PRODUCTION! אתה בסביבת DEV!');
    } else {
      result.warnings.push('⚠️  לא יכול לאמת שה-DATABASE_URL שייך ל-development');
    }
    ...
    if (databaseUrl.includes('RHkhivARk')) {
      ...
      result.valid = false;
      result.errors.push('🚨 DATABASE_URL נראה כמו של DEVELOPMENT! אתה בסביבת PRODUCTION!');
    } else {
      result.warnings.push('⚠️  לא יכול לאמת שה-DATABASE_URL שייך ל-production');
    }
  }
}
...
if (!redisUrl) {
  ...
  if (redisUrl.includes('deQMolmzgWZsqeAkiEpZPFvejfGjenEm')) {
    result.warnings.push('⚠️  Redis משותף בין הסביבות! מומלץ ליצור Redis נפרד לכל סביבה');
...
if (!corsOrigin) {
  ...
  if (environment === 'development') {
    if (corsOrigin.includes('dev.karma-community-kc.com') || corsOrigin.includes('localhost')) {
      ...
      result.warnings.push('⚠️  CORS_ORIGIN לא כולל dev.karma-community-kc.com או localhost');
    }
    ...
    if (corsOrigin.includes('localhost') || corsOrigin.includes('dev.')) {
      result.warnings.push('⚠️  CORS_ORIGIN כולל דומיינים של development בסביבת production!');
    }
  }
}
...
if (!jwtSecret) {
  ...
  result.valid = false;
  result.errors.push(`❌ JWT_SECRET קצר מדי (${jwtSecret.length} תווים). צריך לפחות 32 תווים!`);
} else {
...
if (!googleClientId) {
  result.warnings.push('⚠️  EXPO_PUBLIC_GOOGLE_WEB_CLIENT_ID לא מוגדר (Google OAuth לא יעבוד)');
} else {
