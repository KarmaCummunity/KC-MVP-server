...
async function updateSalaryAndSeniority() {
    try {
        ...
        if (parseInt(null_salary) > 0) {
            ...
            `);
            console.log(`✅ Updated ${salaryResult.rowCount} users with salary = 0\n`);
        } else {
        ...
        if (parseInt(null_seniority) > 0) {
            console.log('📅 Updating seniority_start_date to CURRENT_DATE for NULL values...');
            const seniorityResult = await client.query(`
                UPDATE user_profiles 
                SET seniority_start_date = CURRENT_DATE
                WHERE seniority_start_date IS NULL
            `);
            console.log(`✅ Updated ${seniorityResult.rowCount} users with seniority_start_date = CURRENT_DATE\n`);
        } else {
        ...
        console.log(`   Average salary: ₪${parseFloat(verify.avg_salary || 0).toFixed(2)}`);
