...
async function runMigration() {
    ...
    if (!sqlFile) {
        console.error('❌ Please provide a path to the SQL file: npm run run:sql src/database/your-file.sql');
        process.exit(1);
    }
    ...
    if (connectionString) {
        ...
        pool = new Pool({
            ...
            ssl: process.env.PG_SSL || process.env.POSTGRES_SSL || process.env.PGSSLMODE ? { rejectUnauthorized: false } : undefined,
        });
    }
