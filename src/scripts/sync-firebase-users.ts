...
if (!admin.apps.length) {
  try {
    ...
    if (process.env.FIREBASE_SERVICE_ACCOUNT_KEY) {
      ...
      console.error('❌ FIREBASE_SERVICE_ACCOUNT_KEY or GOOGLE_APPLICATION_CREDENTIALS must be set');
      process.exit(1);
    }
  } catch (error) {
          ...
          try {
            await client.query(
              ...
              [
                ...
                firebaseUser.displayName || normalizedEmail.split('@')[0] || 'User', // name
            ...
            if (insertError.message && insertError.message.includes('google_id')) {
              ...
              created++;
              console.log(`✨ Created user (without google_id): ${normalizedEmail} (${firebaseUser.uid})`);
            } else {
              throw insertError;
            }
          }
