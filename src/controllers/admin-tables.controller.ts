...
import { AdminTablesService, CreateTableDto, UpdateTableDto, CreateRowDto, UpdateRowDto } from '../services/admin-tables.service';
...
export class AdminTablesController {
  constructor(
    private readonly adminTablesService: AdminTablesService,
  ) { }
  ...
  async getTableById(
    ...
    try {
      ...
      const pagination = (includeRowsBool && page && limit) ? {
        page: parseInt(page) || 1,
        limit: parseInt(limit) || 50,
      } : undefined;

      const table = await this.adminTablesService.getTableById(id, includeRowsBool, pagination);
  ...
  async getTableRows(
    ...
    try {
      ...
      const result = await this.adminTablesService.getTableRows(tableId, pagination);
  ...
  @Post(':id/rows')
  async createRow(@Param('id') tableId: string, @Body() dto: CreateRowDto, @Request() req: any) {
    try {
      const userId = req.user?.userId;
      if (!userId) {
        return { success: false, error: 'משתמש לא מזוהה' };
  ...
  async updateRow(
    ...
    try {
      ...
      const row = await this.adminTablesService.updateRow(tableId, rowId, dto, userId);
      return { success: true, data: row };
    } catch (error: any) {
