...
import { IsString, IsUUID, IsEnum, IsOptional, IsNumber, IsDateString, Min, Max, MinLength, MaxLength, IsBoolean } from 'class-validator';
...
export class CreateCommunityGroupChallengeDto {
  ...
  @IsEnum(ChallengeType, { message: 'סוג האתגר חייב להיות BOOLEAN, NUMERIC או DURATION' })
  type: ChallengeType = ChallengeType.BOOLEAN;

  @IsEnum(ChallengeFrequency, { message: 'תדירות האתגר חייבת להיות DAILY, WEEKLY או FLEXIBLE' })
  ...
  @IsOptional()
  @IsEnum(ChallengeDifficulty, { message: 'רמת הקושי חייבת להיות easy, medium, hard או expert' })
  ...
  @IsOptional()
  @IsEnum(GoalDirection, { message: 'כיוון היעד חייב להיות maximize או minimize' })
...
export class GetChallengesFilterDto {
  ...
  @IsEnum(['created_at', 'participants_count', 'title', 'deadline'], {
    message: 'סדר המיון חייב להיות created_at, participants_count, title או deadline'
  })
  sort_by?: string;
